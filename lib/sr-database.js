...
var createTables = function(db) {
  db.run(prepareQuery(srConstants.FOOD_DESCRIPTION_TABLE));
  db.run(prepareQuery(srConstants.FOOD_GROUP_DESCRIPTION_TABLE));
  db.run(prepareQuery(srConstants.LANGUAL_FACTOR_TABLE));
  db.run(prepareQuery(srConstants.LANGUAL_FACTORS_DESCRIPTION_TABLE));
};
