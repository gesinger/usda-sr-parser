...
SrCreator.prototype.fillDb = function (db, cb) {
  console.log('Filling db');

  var numPipes = 0;
  constants.TABLES.forEach((function(table) {
    var lineStream =
      byline.createStream(fs.createReadStream(this.srDir + table.fileName));
    // TODO:
    // Need necessary mappings/constants on create. Might be worth it to have
    // one more stream (possibly)
    var splitStream = new SplitStream();
    var insertStream = new InsertStream(db);
