...
SrCreator.prototype.resolveDb = function (config, cb) {
  var resetDb = (config.reuseDb) ? false : true;

  this.db = new Db('sr', config.dbPath);
  this.db
    .sequelize
    .sync({ force: resetDb })
    .complete((function(err) {
      ...
      if (config.reuseDb && !config.fillTables) {
        console.log('Reusing db at ' + config.dbPath);
        cb();
      } else {
        this.fillDb(config.fillTables, cb);
      }
    }).bind(this));
};
...
SrCreator.prototype.generate = function(config) {
  ...
  config.setMappings(config.mappingsFilePath);
  mappings = config.getMappings();
  Db = require('./models');

  this.resolveDb({
    dbPath: config.dbPath,
    reuseDb: config.reuseDb,
    fillTables: config.fillTables
  }, (function() {
