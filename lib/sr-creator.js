...
SrCreator.prototype.generate =
  ...
  if (reuseDb) {
    if (esExportPath) {
      srObjectifier.objectify(new SrDatabase(dbPath), (function(err, foods) {
        this.generateEsExport(foods, esExportPath, function() {
          console.log('Generated ES export');
        });
      }).bind(this));
    }
  } else {
    this.generateSqliteDb(dbPath, function(srDatabase) {
      ...
      if (esExportPath) {
        srObjectifier.objectify(srDatabase, function(err, foods) {
          this.generateEsExport(foods, esExportPath, function() {
            console.log('Generated ES export');
          });
        });
...
SrCreator.prototype.generateEsExport = function(srDatabase, esExportPath, cb) {
  ...
  /*
  var keyTable = srConstants.FOOD_DESCRIPTION_TABLE.name;

  srDatabase.db.each('SELECT * FROM ' + keyTable, (function(err, row) {
    if (err) {
      cb(err);
    }
    ...
    srConstants.TABLES.forEach(function(table) {
      if (table.name === keyTable) {
        return;
      }

      var i, foreignKey, primaryKey;
      var numCols = table.columns.length;
      for (i=0; i<numCols; ++i) {
        foreignKey = table.columns[i].foreignKey;
        primaryKey = table.columns[i].primaryKey;
        if (foreignKey) {
          food[foreignKey.table]
        }
      }
    });

    this.generateFood(srDatabase, row, function(err, food) {
      if (err) {
        cb(err);
      }
      console.log(food);
    });
  }).bind(this), function(err, numRows) {
    if (err) {
      cb(err);
    }
    console.log('Retrieved ' + numRows + ' from FOOD_DES');
  });
  */
};
