...
Objectifier.prototype._joinRelatedFoodTables =
  ...
  var foodGroupMapping = mappings['FD_GROUP'].name;
  var foodGroupForeignMapping = mappings['FD_GROUP']['FdGrp_Cd'];
  food[foodGroupMapping] = food[foodGroupMapping].values;
  delete food[foodGroupForeignMapping];

  var nutDataMapping = mappings['NUT_DATA'].name;
  var nutDataKeyMapping = mappings['NUT_DATA']['NDB_No'];
  food[nutDataMapping] = this._associatedTableValues(
    food[nutDataMapping],
    nutDataMapping,
    nutDataKeyMapping);
